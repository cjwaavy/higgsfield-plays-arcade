...
// LinkedIn is no longer distributed by simple-icons (brand policy), so the classic
// glyph path is inlined here.
const LINKEDIN_PATH =
...
const BRAND_BY_PLAY = {
  'meta-ads': { title: 'Meta', path: siMeta.path },
  'airbnb-walkthroughs': { title: 'Airbnb', path: siAirbnb.path },
  'real-estate-ads': { title: 'Zillow', path: siZillow.path },
  'b2b-commercials': { title: 'LinkedIn', path: LINKEDIN_PATH },
  'website-design': { title: 'Framer', path: siFramer.path },
  'yt-shorts': { title: 'YouTube Shorts', path: siYoutubeshorts.path },
  'yt-kids': { title: 'YouTube Kids', path: siYoutubekids.path },
}

export function BrandMark({ playId }) {
  const brand = BRAND_BY_PLAY[playId]
  if (!brand) return null
  return (
    <svg viewBox="0 0 24 24" role="img" aria-label={brand.title} className="lockup-mark">
      <path d={brand.path} fill="currentColor" />
    </svg>
  )
}

export function HiggsfieldMark() {
  return (
    <svg viewBox="0 0 20 20" role="img" aria-label="Higgsfield" className="lockup-mark hf">
      <path d={HIGGSFIELD_PATH} fill="currentColor" />
    </svg>
  )
}
